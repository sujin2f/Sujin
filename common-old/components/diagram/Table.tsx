...
export const Table = (props: Props) => {
    ...
    const [[width, height, x1, y1, x2, y2], setSize] = useState<[number, number, number, number, number, number]>([
        0, 0, 0, 0, 0, 0,
    ])
    ...
    useEffect(() => {
        const xPos = textRef.current.reduce(
            (acc, ref) => {
                return [...acc, acc[acc.length - 1] + ref.getBoundingClientRect().width]
            },
            [0],
        )
        setXPos(xPos)
    ...
    return (
        <g id={props.id}>
            ...
            {!isNaN(width) && (
                <>
                    <rect x={x1} y={y1} width={width} height={height} strokeWidth={1} stroke="black" fill="white" />
                    {/* Vertical Lines */}
            ...
            {textArr.map((col, colIndex) => {
                const getRef = (element: SVGTextElement) => textRef.current.push(element) as unknown as undefined
                ...
                return (
                    ...
                    >
                        {col.map((cell, cellIndex) => (
                            <tspan x={x} dy={paddings + fontSize} key={`cell-${colIndex}-${cellIndex}`}>
                                {cell || ' '}
                            </tspan>
                        ))}
                    </text>
