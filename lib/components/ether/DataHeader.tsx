...
/* Helpers */
import { periodicTable } from '@common/constants/ether'
    ...
    return (
        <aside className="header--ether">
            ...
            <Row dom="nav" className="header--ether__atom">
                <Column small={4}>
                    {prev && <Link href={`/ether/data/${type}/${prev.number}/1`}>{prev.name}</Link>}
                </Column>
                <Column small={4}>
                    <h1 className="--center">
                ...
                <Column small={4} className="--right">
                    {next && <Link href={`/ether/data/${type}/${next.number}/1`}>{next.name}</Link>}
                </Column>
            </Row>
            {/* Type / Term Selection */}
