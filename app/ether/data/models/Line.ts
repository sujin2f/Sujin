...
import { jouleToEv, ROW_HEAD } from '@common/constants/ether'
...
export class Line {
    ...
    private get shift(): number {
        ...
        this._shift = atom.ionization_energies[this.first.ion - 1] * jouleToEv - this.ratio
        return this._shift
    }
    ...
    private get ratioFromPoints() {
        const first = this.first.energy === 0 ? this.spectra[this.first.position] : this.first
        if (!first) {
            return NaN
        }
    ...
    get k() {
        ...
        this._k = Math.sqrt(this.ratio / (this.ratio - first.energy + this.shift)) - first.position
        return this._k
    }
    ...
    get fix() {
        ...
        this._fix = -0.000000307 * Math.pow(this.first.ion - 1, 2.053) + 0.999931
        return this._fix
    }
    ...
    private getRydberg() {
        ...
        this._rydberg = this.spectra.map((item, index) => {
            ...
            return ratio * (1 / Math.pow(item.position - 1 + this.k, 2) - 1 / Math.pow(item.position + this.k, 2))
        })
        return this._rydberg
    }
        ...
        ROW_HEAD.forEach((row, index) => {
            switch (row) {
                ...
                case 'Rydberg':
                    ...
                    break
                case 'Comparison':
                    this._value[index] = this.getComparison(ROW_HEAD.indexOf('Diff'), ROW_HEAD.indexOf('Rydberg'))
                    break
            }
        })
