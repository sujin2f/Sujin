...
import type { ISpectrum, SortType } from '@common/types'
...
export class Term {
    ...
    public get maxColumn() {
        return Math.max(...this.rawItems.map((line) => line.length).filter((l) => l))
    }

    public map<T>(callbackfn: (value: Line, index: number) => T) {
        return [this.radial, this.linier, ...this.lines].filter((line) => line && line.length !== 0).map(callbackfn)
    }

    private setRadial() {
        for (let i = 0; i < this.rawItems.length; i++) {
            if (this.rawItems[i] && this.rawItems[i].filter((item) => item)[0].orbital === 's') {
    ...
    private setLinier() {
        this.getDiagonal(this.maxColumn, this.maxColumn + 1).forEach((item) => this.linier.push(item))
    }

    private sortOrbital() {
        this.rawItems
            .filter((items) => items && items.filter((s) => s)[0].orbital !== 's')
            .forEach((line, index) => {
