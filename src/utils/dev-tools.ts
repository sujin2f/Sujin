 ...
 * CONSTANT_CASE
 */
export const constantCase = (texts: string[]): string => texts.map((text) => text.toUpperCase()).join('_');

/*
 * dot.case
 */
export const dotCase = (texts: string[]): string => texts.map((text) => text.toLowerCase()).join('.');

/*
 * param-case
 */
export const paramCase = (texts: string[]): string => texts.map((text) => text.toLowerCase()).join('-');

/*
 * PascalCase
 */
export const pascalCase = (texts: string[]): string => texts.map((text) => ucfirst(text)).join('');

/*
 * path/case
 */
export const pathCase = (texts: string[]): string => texts.map((text) => text.toLowerCase()).join('/');

/*
 * snake_case
 */
export const snakeCase = (texts: string[]): string => texts.map((text) => text.toLowerCase()).join('_');

/*
 * Title Case
 */
export const titleCase = (texts: string[]): string => texts.map((text) => ucfirst(text)).join(' ');
