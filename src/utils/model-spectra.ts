...
export class Spectrum {
    ...
    get comparison2() {
        const rydberg = this.rydberg2
        if (isNaN(rydberg) || isNaN(this.energy)) {
            return NaN
        }
        // When the value is larger than 1, rydberg value is larger than the energy
        return rydberg / this.energy
    }
    ...
    get rydberg2() {
        const ratio = this.parent?.ratio
        const k = this.parent?.k
        if (!ratio || !k || this.position === 1) {
            return NaN
        }
        return ratio * (1 - 1 / Math.pow(this.position + k, 2))
    }
}
...
export class Lines {
    ...
    _top = [0, 2372.3, 7298.1, 14848.7, 25025.8]
    ...
    get ratio() {
        ...
        //     return this._ratio[this.points[0].ion] * this.fix
        // }
    ...
    get k() {
        ...
        return (
            Math.sqrt(this.ratio / (this.ratio - this.points[0].energy + 0)) - 2
        )
    }
